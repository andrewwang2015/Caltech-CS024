...
addr_t get_offset_in_block(cache_t *p_cache, addr_t address) {
     ...
     * block_size). 
     */
    uint32_t bitmask = p_cache->block_size - 1;

    /* Use bitwise AND to only get the lower b bits of the address. */
    addr_t offset = address & bitmask;
    return offset;
}
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    *offset = get_offset_in_block(p_cache, address);
    /* 
     * Shift address to the right so that the bottom bits correspond to 
     * set index. 
     */
    address >>= p_cache->block_offset_bits;

    /* Bitwise AND with mask to get bottom set index bits. */
    *set = address & (p_cache->num_sets - 1);

    /* Shift address again to get bottom bits to correspond to tag. */
    address >>= p_cache->sets_addr_bits;
    *tag = address;
}
...
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
    /*
     * Set the block_offset bits to 0, so we first shift right to clear them
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
                                      addr_t tag, addr_t set_no) {
    /* Initially set res to be tag. */
    addr_t res = tag;

    /* Left shift to make room for the set-idx. */
...
#else
    /* i_victim is only used in RANDOM_REPLACEMENT_POLICY. */
    (void) i_victim;

     /* Least Recently Used (LRU) policy to evict a block. */
    uint64_t minimum_time = clock_tick();
    int num_lines = p_set->num_lines;
    cacheline_t *lines = p_set->cache_lines;

    /* Iterate through all lines. */
    for (int i = 0; i < num_lines; i++) {
        /* If we come across an invalid line, return that immediately. */
        if (!lines[i].valid) {
            victim = &lines[i];
            break;
        } else {
            /* Keep track of the line with the smallest access-time value. */
            if (lines[i].time < minimum_time) {
                victim = &lines[i];
                minimum_time = lines[i].time;
            }
        }
    }
#endif
