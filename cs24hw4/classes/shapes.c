...
void Shape_class_init(Shape_Class *class) {
    /* getVolume for Shape_Class is abstract. */
...
Box_Data * new_Box(float L, float W, float H, float D) {
    Box_Data *newBox = malloc(sizeof(Box_Data));
    Box_init(newBox, &Box, L, W, H, D);
}
...
Sphere_Data * new_Sphere(float R, float D) {
    Sphere_Data *newSphere = malloc(sizeof(Sphere_Data));
    Sphere_init(newSphere, &Sphere, R, D);
}
...
float Sphere_getVolume(Sphere_Data *this) {
    return (4.0 / 3) * M_PI * this->radius * this->radius * this->radius;
}
...
Cone_Data * new_Cone(float BR, float H, float D) {
    Cone_Data *newCone = malloc(sizeof(Cone_Data));
    Cone_init(newCone, &Cone, BR, H, D);
}
...
float Cone_getVolume(Cone_Data *this) {
    return (1.0 / 3) * M_PI * this->base_radius * this->base_radius 
        * this->height;
}
